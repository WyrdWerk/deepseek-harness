...
A command feedback remark is log-only: it never enters model context or derived history. Capture itself is independent of any reporting backend; with no telemetry backend mounted, a recorded remark simply stays in the local session log.
