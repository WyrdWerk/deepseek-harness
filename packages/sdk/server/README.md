...
- **Automatic adapter mounting is pi-ai-based** — `initialize` can reuse any pre-registered model adapter, but its only fallback mounts `dsh-llm-pi-ai` with the `openai` catalog route.
