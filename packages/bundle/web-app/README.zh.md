...
- **`--tailscale` 会改写本节点上该监听端口的 `tailscale serve` HTTPS 规则**：只替换该端口的规则；在共享节点上首次发布前，用 `tailscale serve status --json` 做快照。
