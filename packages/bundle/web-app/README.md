...
- **`--tailscale` mutates this node's `tailscale serve` HTTPS rule for the listen port** — it replaces that port's rule only; snapshot with `tailscale serve status --json` before the first publish on a shared node.
