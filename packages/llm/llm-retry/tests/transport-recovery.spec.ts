...
describe('bounded retry through the real pi-ai HTTP/SSE adapter', () => {
  ...
  it('retries a clean partial EOF as TRANSPORT without committing the partial content', async () => {
    const server = await start(['partial_eof', 'success'], {
      apiKey: 'mock-key',
      partialText: 'discarded clean eof',
      chunkSize: 100,
      successText: 'recovered from partial eof',
    })
    context = await harness(server.baseURL)
    ...
    expect(agent.session.events.filter(event => event.type === 'llm/retry').map(event => event.data.failure.code))
      .toEqual(['TRANSPORT'])
    expect(finalAssistantText(agent)).toBe('recovered from partial eof')
  })
